...
def run_recoverr_verifyingevidences(
    ...
    if 'objdet' in visual_tools:
        ...
        for object in image_objects:
            ...
            object_evidence = {
                    ...
                    'relevance': 0.0,
                    'is_reliable': True if object_presence_confidence >= 1-desired_risk else False,
                    'is_relevant': False,
                }
            if object_presence_confidence >= 1-desired_risk:
    ...
    if 'regcapdb' in visual_tools:
        ...
        for region in region_captions:
            region_presence_question = f"Does the image contain {region}?"
            region_presence_confidence = vlm_model.get_answer_confidence(
                raw_image=query_details['image'], 
                questions=[region_presence_question],
                answers=['yes']
            )[0][0]
            region_statement = f"The image contains {region}."
            region_evidence = {
                    'question': region_presence_question,
                    'answer': 'yes',
                    'vlm_conf': region_presence_confidence,
                    'statement': region_statement,
                    'prediction_entailment_conf': 0.0,
                    'counterfactual_prediction_entailment_conf': 0.0,
                    'relevance': 0.0,
                    'is_reliable': True if region_presence_confidence >= 1-desired_risk else False,
                    'is_relevant': False,
                }
            if region_presence_confidence >= 1-desired_risk:
                region_presence_entailment_conf = llm_model.get_entailment_confidence(
                    premise=region_statement,
                    hypothesis=directvqa_hypothesis
                )
                region_absence_premise = f"The image does not contain {region}."
                region_absence_entailment_conf = llm_model.get_entailment_confidence(
                    premise=region_absence_premise,
                    hypothesis=directvqa_hypothesis
                )
                region_relevance = np.abs(region_presence_entailment_conf - region_absence_entailment_conf)
                region_evidence['prediction_entailment_conf'] = region_presence_entailment_conf
                region_evidence['counterfactual_prediction_entailment_conf'] = region_absence_entailment_conf
                region_evidence['relevance'] = region_relevance
                region_evidence['is_relevant'] = True if region_relevance >= defeasibility_delta else False
                if do_print:
                    print("")
                    print(region_statement)
                    print(f"P({directvqa_hypothesis} | {region_statement}) = {region_presence_entailment_conf:.4f}")
                    print(f"P({directvqa_hypothesis} | {region_absence_premise}) = {region_absence_entailment_conf:.4f}")
    
                reliable_evidences.append(region_evidence)
                if region_relevance >= defeasibility_delta:
                    reliable_and_relevant_evidences.append(region_evidence)
            all_evidences.append(region_evidence)
    #endregion
    if do_print:
        print("-"*100)
    ...
    if len(reliable_and_relevant_evidences) > 1:
...
def main():
    ...
    parser.add_argument("--experiments_dir", type=str, default='/net/nfs.cirrascale/mosaic/tejass/experiments/recoverr/recoverr_012824')
